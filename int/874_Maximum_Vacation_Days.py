...
class Solution:
    ...
    def maxVacationDays(self, flights, days):
        ...
        N = len(flights)
        K = len(days[0])
        # dp = [int(days[i][0]) for i in xrange(N)] 
        dp = [0] * N
        dp[0] = days[0][0]
        for i in xrange(1,N):
            if flights[0][i] == 1:
                ...
                dp[i] = float('-inf')
        dpt = dp
        for curr_day in range(1,K):
            for terminal in range(N):
                tmp_max = float('-inf')
                for departure in range(N):
                    if flights[departure][terminal] or departure==terminal:
                        tmp_max = max(tmp_max, dp[departure] + int(days[terminal][curr_day]))
                dpt[terminal] = tmp_max
            dp = dpt
        return max(dp)
